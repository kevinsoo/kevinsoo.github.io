...
#     facet_wrap(aes(facets = cyl), nrow=length(unique(mpg$cyl)))
```
...
# mpg %>%
#     ggplot(aes(x = displ, y = hwy)) + 
#     geom_jitter(aes(color = as.factor(cyl)), alpha = .3, width = .05, height = .3) +
#     stat_smooth(method = "lm", aes(color = as.factor(cyl))) +
#     facet_grid(~ year) +
...
#     summarise(N = n(), M.hwy = mean(hwy), SD.hwy = sd(hwy)) %>%
#     ggplot(aes(x = fct_reorder(manufacturer, -M.hwy), y = M.hwy)) +
#     geom_errorbar(aes(ymin = M.hwy - SD.hwy, ymax = M.hwy + SD.hwy), width = .1) +
...
# SOLUTION
# mpg %>%
#     ggplot(aes(x = fct_reorder(manufacturer, -hwy), y = hwy)) +
#     # geom_violin() +
#     geom_boxplot() +
